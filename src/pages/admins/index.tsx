...
import {TableActions, TableHeader, TableSkeleton, TableFooter} from "../../components";
import {useUsers} from "../../hooks/user";

const AdminList: NextPage = () => {
    const [currentPage, setCurrentPage] = useState(0);
    const {data, isLoading} = useUsers(ITEMS_PER_PAGE, currentPage, "staff")
        ...
        <section className='p-8'>
            <div className='space-y-8'>
                <TableHeader
                    ...
                    url={`${appRoutes.ROUTE_ADMINS}/add`}
                    disabled={isLoading}/>
                            ...
                            <Tbody>
                                {data.users.map(({firstName, lastName, avatarId, email, phoneNumber, userId}, key) => (
                                    <Tr key={key} className='transition-all hover:bg-slate-100'>
                                        ...
                                        <Td isNumeric>
                                            <TableActions
                                                detailsUrl={`${appRoutes.ROUTE_ADMINS}/${userId}`}
                                                editUrl={`${appRoutes.ROUTE_ADMINS}/${userId}/edit`}/>
                                        </Td>
                                    </Tr>
                                ))}
                            </Tbody>
